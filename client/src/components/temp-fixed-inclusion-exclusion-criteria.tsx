...
import { AIGenerationStatus, SectionStatus, GenerationStatus } from "@/components/ai-generation-status"
...
const InclusionExclusionCriteria: React.FC<InclusionExclusionCriteriaProps> = ({ protocol, setProtocol }) => {
  ...
  // Setup edit function
  const handleEditCriterion = (criterion: any, type: string) => {
    // This would be implemented for editing functionality
    console.log("Edit criterion", criterion, type)
  }

  // Handle toggling rationale display
  const toggleRationale = (id: number, type: string) => {
    // This would be implemented for toggling rationale
    console.log("Toggle rationale", id, type)
  }
  ...
  // Helper to get appropriate impact colors
  ...
  // Render a criterion card
  const renderCriterionCard = (criterion: any, type: string, index: number) => (
    <li key={`${type}-${criterion.id}`} className="relative group border border-[#e9ecef] rounded-md overflow-hidden">
      {/* Impact level left border indicator */}
      <div 
        className="absolute left-0 top-0 bottom-0 w-2.5 z-10" 
        style={{ 
          backgroundColor: getImpactColor(criterion.impact).dot 
        }}
      ></div>
      ...
      <div className="grid grid-cols-[1fr_120px] bg-white">
        ...
        <div className="border-l border-[#e9ecef] flex items-center justify-center gap-1.5 bg-[#f8f9fa]">
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-full flex-shrink-0"
              style={{ backgroundColor: getImpactColor(criterion.impact).dot }}
            ></div>
            <div 
              className="px-2 py-1 rounded text-xs font-medium"
              style={{ 
                backgroundColor: getImpactColor(criterion.impact).bg,
                color: getImpactColor(criterion.impact).text 
              }}
            >
              {criterion.impact}
            </div>
      ...
      <Button
        ...
        onClick={() => handleDeleteClick(criterion.id, type as "inclusion" | "exclusion")}
      >
        <TrashIcon size={14} />
      </Button>
    </li>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-md border border-[#dee2e6] overflow-hidden">
            ...
            <div className="p-4">
              <ul className="space-y-6">
                {parsedInclusionCriteria.map((criterion: any, index: number) => 
                  renderCriterionCard(criterion, "inclusion", index)
                )}
              </ul>
          ...
          <div className="bg-white rounded-md border border-[#dee2e6] overflow-hidden">
            ...
            <div className="p-4">
              <ul className="space-y-6">
                {parsedExclusionCriteria.map((criterion: any, index: number) => 
                  renderCriterionCard(criterion, "exclusion", index)
                )}
              </ul>
